...
Validates pipeline stages and returns config status strings.
...
```python
# ❌ BAD — late validation, no guard clause
def validate_pipeline_bad(stages: list[str]) -> str:
    result = "valid_config"
    for stage in stages:
        if stage not in {"build", "test", "deploy", "notify"}:
            result = "invalid_config"  # Mutation, hidden state
    return result

# ✅ GOOD — guard clause, early exit, pure function
def validate_pipeline_good(stages: list[str]) -> str:
    ALLOWED_STAGES = {"build", "test", "deploy", "notify"}
...
Validates pipeline stages against a fixed set of allowed stages and returns config status.
...
```python
# ❌ BAD — hardcoded allowed set buried in logic
def validate_pipeline_bad(stages: list[str]) -> str:
    for stage in stages:
        if stage not in {"build", "test", "deploy", "notify"}:
...
# ✅ GOOD — constant defined at top, guard clause, fail fast
def validate_pipeline_good(stages: list[str]) -> str:
    ALLOWED_STAGES = {"build", "test", "deploy", "notify"}
