...
class LLMRanker {
    ...
    async rankCandidates(task, candidates) {
        ...
        // Parse response
        const rankings = this.parseRankingResponse(response, limitedCandidates.map((c) => c.metadata.name));
        // Convert to SelectedSkill format
        const selectedSkills = rankings.map((ranking, index) => ({
            ...
            const response = await fetch('https://api.openai.com/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'You are a precise skill router. Always output valid JSON.',
                        },
