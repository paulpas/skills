...
export class SkillCompressor {
  private recipes: Map<number, CompressionRecipe> = new Map();
  private readonly MIN_TOKEN_THRESHOLD = 100;
   ...
   */
  public compress(content: string, level: number): CompressedSkill {
    ...
    try {
      ...
      for (let i = 1; i <= level; i++) {
        ...
        if (!recipe) break; // Stop if recipe doesn't exist
   ...
   */
  private applyRecipe(content: string, recipe: CompressionRecipe): string {
    let result = content;
   ...
   */
  public estimateTokenSavings(content: string, level: number): TokenEstimate {
    const originalTokens = this.estimateTokens(content);
   ...
   */
  public shouldCompress(content: string): boolean {
   ...
   */
  public getRecipe(level: number): CompressionRecipe | null {
   ...
   */
  public getAllLevelDescriptions(): Array<{ level: number; description: string }> {
