 ...
 */
export class LLMSkillCompressor {
  ...
  constructor(llmClient: LLMClient) {
    ...
    this.logger = new Logger('LLMSkillCompressor');
  }
  ...
  async compressSkill(
    ...
    try {
      ...
      const briefVersion =
        brief.status === 'fulfilled' && brief.value
          ? brief.value
          : this.createFailedVersion('brief', originalContent);

      const moderateVersion =
        moderate.status === 'fulfilled' && moderate.value
          ? moderate.value
          : this.createFailedVersion('moderate', originalContent);

      const detailedVersion =
        detailed.status === 'fulfilled' && detailed.value
          ? detailed.value
   ...
   */
  private async generateCompressedVersion(
    skillName: string,
    originalContent: string,
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a skill documentation optimizer for agentic AI tools. ' +
...
Return ONLY the compressed Markdown content (including YAML frontmatter if present). Do not add explanations or comments.`;
   ...
   */
  private ensureYAMLFrontmatter(compressedContent: string, yamlFrontmatter: string): string {
   ...
   */
  private getMaxTokensForStyle(style: 'brief' | 'moderate' | 'detailed'): number {
    const maxTokens = {
      brief: 600,
      moderate: 1200,
      detailed: 2000,
